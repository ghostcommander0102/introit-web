...
export default function Paris({
    ...
    const [skillPos, setSkillPos] = useState({ pos: 683, page: 1 })
    ...
    // const [cv, setCV] = useState(props.cv)styles
    ...
    useEffect(() => {
        function updateSize() {
            ...
            setDeviceRatio(1 / (widthRatio > heightRatio ? widthRatio : heightRatio) / window.devicePixelRatio)
        }
        ...
        setTimeout(async () => {
            html2canvas(document.querySelector("#resume"), {
                ...
                if (page == 1) {
                    ...
                    context.drawImage(linkedImg, 294 * deviceRatio, 188 * deviceRatio, 14 * deviceRatio, 14 * deviceRatio);
                    context.drawImage(dribbleImg, 324 * deviceRatio, 188 * deviceRatio, 14 * deviceRatio, 14 * deviceRatio);
                    context.drawImage(instagramImg, 354 * deviceRatio, 188 * deviceRatio, 14 * deviceRatio, 14 * deviceRatio);
                    context.drawImage(whatsappImg, 384 * deviceRatio, 188 * deviceRatio, 14 * deviceRatio, 14 * deviceRatio);
                    context.drawImage(twitterImg, 414 * deviceRatio, 188 * deviceRatio, 14 * deviceRatio, 14 * deviceRatio);
    ...
    const getTextColor = () => {
        return (type == "dark" ? "#EFF1F8" : (type == "light" ? "#2F2F3D" : "#242435"))
    }

    const getTextColor1 = () => {
        return (type == "dark" ? "#9496B3" : (type == "light" ? "#A4AABE" : "#6B7598"))
    }
    ...
    const generateProfile = async (report) => {
        ...
        report.text("$ " + (cv.minSalary ? cv.minSalary : 0) + " - " + (cv.maxSalary ? cv.maxSalary : 0) + " monthly", 288 * ratio, (543 + report.getTextDimensions("$ " + (cv.minSalary ? cv.minSalary : 0) + " - " + (cv.maxSalary ? cv.maxSalary : 0) + " monthly").h) * ratio)
    ...
    const getEducationContent = (page) => {
        if (cv.userEducations && cv.userEducations.length > 0) {
            return (
                <>
                    {eduPos.page == page ? <div className={styles.componentTitle} style={{ top: eduPos.pos, left: 50 }}>Education</div> : <></>}
                    ...
                    {skillContent.map((item) => {
                        ...
                        for (let i = 0; i < 5; i++) {
                            if (cv.userSkils[item.index].percentage >= (i + 1) * 20) {
                                percent.push(<NextImage src={getPercentageImageURL("full")} width={12} height={12} alt="" />)
                            } else if (i * 20 < cv.userSkils[item.index].percentage && cv.userSkils[item.index].percentage < (i + 1) * 20) {
                                percent.push(<NextImage src={getPercentageImageURL("half")} width={12} height={12} alt="" />)
                            } else {
                                percent.push(<NextImage src={getPercentageImageURL("blank")} width={12} height={12} alt="" />)
                            }
                        }
                        return (
                            item.page == page ?
    ...
    const getExperienceContent = (page) => {
        if (cv.userExperiences && cv.userExperiences.length > 0) {
            return (
                <>
                    {expPos.page == page ? <div className={styles.componentTitle} style={{ top: expPos.pos, left: 50 }}>Experience</div> : <></>}
    ...
    const getLanguageContent = (page) => {
        if (cv.userLanguages && cv.userLanguages.length > 0) {
            return (
                <>
                    {langPos.page == page ? <div className={styles.componentTitle} style={{ left: 50, top: langPos.pos }}>Languages</div> : <></>}
    ...
    const getInternshipsContent = (page) => {
        if (cv.userInterships && cv.userInterships.length > 0) {
            return (
                <>
                    {internPos.page == page ? <div className={styles.componentTitle} style={{ left: 50, top: internPos.pos }}>Internships</div> : <></>}
    ...
    const getExtraActivites = (page) => {
        if (cv.userExtraActivities && cv.userExtraActivities.length > 0) {
            return (
                <>
                    {extraPos.page == page ? <div className={styles.componentTitle} style={{ left: 50, top: extraPos.pos }}>Extra-curricular Activities</div> : <></>}
            ...
            return (
                <>
                    {hobbiesPos.page == page ?
                        <div className={styles.componentTitle} style={{ left: 50, top: hobbiesPos.pos }}>Hobbies</div>
                        : <></>}
    ...
    const getAdditional = (page) => {
        if (cv.additionalInfo) {
            return (
                <>
                    {addPos.page == page ?
                        <div className={styles.componentTitle} style={{ left: 50, top: addPos.pos }}>Additional information</div>
                        : <></>}
    ...
    const getAwards = (page) => {
        if (cv.userAwards && cv.userAwards.length > 0) {
            return (
                <>
                    {honorPos.page == page ? <div className={styles.componentTitle} style={{ left: 50, top: honorPos.pos }}>HONORS & AWARDs</div> : <></>}
    ...
    return (
        <>
            <div className={styles.resumeContent + " " + styles.paris + " " + getStyle()}>
